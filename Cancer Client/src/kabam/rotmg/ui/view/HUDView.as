...
import com.company.assembleegameclient.ui.LootOpenedUI;
...
public class HUDView extends Sprite implements UnFocusAble {
    ...
    private function Showuibox():void {
        ...
        this.LootOpenedUISignal.x -= this.LootOpenedUISignal.width;
        this.LootOpenedUISignal.y = this.characterDetails.y;
        this.LootOpenedUISignal.alpha = 0;
        addChild(this.LootOpenedUISignal);
    }
    public function Secretmenu(_arg1:uint):void {
        this.LootOpenedUISignal.setItemBitmap(_arg1);
        if (!this.stage.contains(this.LootOpenedUISignal)){
            addChild(this.LootOpenedUISignal);

        }
        this.LootOpenedUISignal.alpha = 0;
        new GTween(this.LootOpenedUISignal, 2, {"alpha": 1});
        new GTween(this.LootOpenedUISignal, 0.001, {"x": - this.equippedGrid.x * 25});
        new GTween(this.LootOpenedUISignal, 700, {"y": this.characterDetails.y});
